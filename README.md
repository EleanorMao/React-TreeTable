...
* onArrowClick[Function(opened, data，parent)] 点击展开按钮的时候会把数据丢给你可以处理，会返回`opened`,`data`, `parent`三个参数和一个callback`callback`，务必要`callback(data)`(为了处理异步的情况)。`opened`为`false`的时候意味着当前叶子节点是关闭状态，如果是`true`说明当前叶子节点是展开的。如果有子节点的数据是通过异步请求搞进去的这种需求，请通过引用的原理来处理。
